...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/|/|/" name="service-impl-fill-children.xsl">
		...
		<xsl:variable name="fillChildrenMethod" select="helper:getMethodAnnotation($interfaceName, $methodNode/@name, 'clear.cdb.extjs.annotations.CX_JSFillChildrenMethod')"/>
				...
				if (item!=null) {
					...
					if (item.<xsl:value-of select="@readMethod"/>() != null) {
						...
						item.<xsl:value-of select="@writeMethod"/>(parentDTO<xsl:value-of select="position()"/>);
								...
								<xsl:otherwise>
					Object parentValue<xsl:value-of select="position()"/> = IdentityRack.getIdentity("<xsl:value-of select="helper:replaceAll($fxManyToOneParent, '.$', '.')"/>", "<xsl:value-of select="$fxManyToOneProperty"/>", item.<xsl:value-of select="@readMethod"/>());
