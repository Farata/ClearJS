...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/|/|/" name="service-interface-fill.xsl">
 	...
 	java.util.List&lt;flex.data.ChangeObject&gt; <xsl:value-of select="$methodNode/@name"/>_sync(java.util.List&lt;flex.data.ChangeObject&gt; items) throws Exception;
	...
	void <xsl:value-of select="$methodNode/@name"/>_doCreate(flex.data.ChangeObject changeObject);
	
	void <xsl:value-of select="$methodNode/@name"/>_doUpdate(flex.data.ChangeObject changeObject);

	void <xsl:value-of select="$methodNode/@name"/>_doDelete(flex.data.ChangeObject changeObject);
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>