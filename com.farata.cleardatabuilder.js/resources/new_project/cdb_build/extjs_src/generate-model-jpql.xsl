...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/|/|/" name="generate-model-jpql.xsl">
		...
		<xsl:variable name="jpqlMethodNode"
			select="$methodNode/annotations/annotation[@name='clear.cdb.js.annotations.CX_JSJPQLMethod']" />
		<xsl:variable name="types"
			select="$jpqlMethodNode/method[@name='query']/query" />
		<xsl:variable name="updateInfo"
			select="$methodNode/annotations/annotation[@name='clear.cdb.js.annotations.CX_UpdateInfo'] | $jpqlMethodNode/method[@name='updateInfo']/value/annotation" />
		...
		<xsl:variable name="annotatedType"
			select="annotated-types/annotated-type[@name=$interfaceName][1]" />

		<xsl:variable name="updateEntityIdPropNames">
			<xsl:variable name="keyPropsValue"
				select="$updateInfo/method[@name='keyPropertyNames']/@value" />
			<xsl:choose>
				<xsl:when
					test="(not($keyPropsValue) or $keyPropsValue='') and $updateEntity">
					<xsl:variable name="updateEntityProps"
						select="helper:getBeanProperties($updateEntity)" />
					<xsl:for-each select="$updateEntityProps/property">
						<xsl:variable name="updateEntityId"
							select="helper:getBeanPropertyAnnotation($updateEntity, @name, 'javax.persistence.Id')" />
						<xsl:variable name="updateEntityEmbId"
							select="helper:getBeanPropertyAnnotation($updateEntity, @name, 'javax.persistence.EmbeddedId')" />
...
<xsl:text/>Ext.define('<xsl:value-of select="$appName"/>.model.<xsl:value-of select="$rootPackage"/>.<xsl:value-of select="$dtoName"/>',{
	...
	<xsl:variable name="mappedEntity"
		select="helper:genDTOtoEntity(concat($rootPackage, '.', $dtoName))" />
	<xsl:for-each select="$types/types/type">
		<xsl:variable name="alias" select="@alias" />
		<xsl:variable name="isKey"
			select="$keyPropertyNames/element[@value=$alias]" />

		<xsl:if test="$mappedEntity">
			<xsl:variable name="mappedEntityProps"
				select="helper:getBeanProperties($mappedEntity)" />
			<xsl:variable name="propName" select="@alias" />

			<xsl:variable name="propNode"
				select="$mappedEntityProps/property[@name=$propName]/@name" />
			<xsl:if test="$propNode">
				<xsl:variable name="manyToOne"
					select="helper:getBeanPropertyAnnotation($mappedEntity, $propName, 'javax.persistence.ManyToOne')" />
				<xsl:variable name="propType" select="@name" />
				<xsl:if test="$manyToOne/method[@name = 'targetEntity']">
					<xsl:variable name="refDtoProps"
						select="helper:getBeanProperties($propType)" />
					<xsl:variable name="keyColumn">
						<xsl:for-each select="$refDtoProps/property">
							<xsl:variable name="fxKeyColumn"
								select="helper:getBeanPropertyAnnotation($propType, @name, 'javax.persistence.Id')" />
							<xsl:variable name="fxEmbKeyColumn"
								select="helper:getBeanPropertyAnnotation($propType, @name, 'javax.persistence.EmbeddedId')" />
	...
	<xsl:if test="$mappedEntity">
		...
		<xsl:for-each select="$mappedEntityProps/property">
			<xsl:variable name="oneToMany"
				select="helper:getBeanPropertyAnnotation($mappedEntity, @name, 'javax.persistence.OneToMany')" />
			<xsl:if test="$oneToMany/method[@name = 'targetEntity']">
				<xsl:variable name="typeName"
					select="helper:replaceEntitiesWithGenDTOs(@type)" />
