...
Ext.define('Clear.transaction.BatchManager', {
		...
		for ( i=_registry.length-1; i>=0; i-- ) {
			...
			if (records.length > 0) {
				...
				batchMember = Ext.create('Clear.transaction.BatchMember', {
					...
					//,(store.autoSyncSubtopic!=null && store.autoSyncSubtopic!="")?store.autoSyncSubtopic:store.argumentString
				});
	...
	//		stateMap = me.stateMap = Ext.create("Ext.util.HashMap"),
		...
		if( _registry) {
			for (i=0; i< _registry.length; i++ ) {
				store = _registry[i].store;
				key =  this.getStoreKey(store.proxy.api.read, store.argumentString);
...
//				stateMap[key] = token;
	 ...
	 * @private
	 */
    getStoreKey: function(store, argumentString) {
    	return 
    		this.getDirectActionName(store, 'read') + "." +
    		this.getDirectMethodName(store, 'read') + "."  + 
    		argumentString;
    },    
	/*
	 * @private
	...
	onResult: function (result) {
		 ...
		 for (var i=0; i< result.length; i++ ) {
			 ...
			 key = me.getStoreKey(
				batchMember.className + "." + me.ignoreSubmethod(batchMember.methodName), batchMember.autoSyncSubtopic
			 );
	...
	//if (store.syncRequired) {
		...
		Ext.each(store, function(item){
			if (item.childStores && item.childrenModified) {
				childStores = item.childStores;
				childRanking =  item.childRanking;
				for (i=0; i<childStores.length; i++) {
		...
		Ext.each(store.getRemovedRecords(), function(item){			
			if (item.childStores && item.childrenModified) {
				childStores = item.childStores;
				for (i=0; i<childStores.length; i++) {
					me.registerWithChildren({
